...
func (g *GitHubClient) UploadFile(ctx context.Context, path string) (string, error) {
	// Read file content
	...
	uniqueFileName := fmt.Sprintf("%s-%s", timestamp, fileName)

	// Complete file path
	fullPath := g.path + uniqueFileName
